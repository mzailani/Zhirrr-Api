...
loghandler = {
    notparam: {
        ...
        getApikey: 'gak punya apikey? chat gw aja yaaa di wa.me/628881277513'
    },
    notkey: {
        status: false,
        creator: `${creator}`,
...
router.get('/textmaker/new51', async (req, res, next) => {
        var theme = req.query.theme,
             text = req.query.text,
             text2 = req.query.text2,
             text3 = req.query.text3,
	...
	if(apikeyInput != 'ZailaniGans') return res.json(loghandler.invalidKey)
        if (!theme) return res.json(loghandler.nottheme)
        if (theme != '8-bit' && theme != 'typography') return res.json(loghandler.notheme)
        if (!text) return res.json(loghandler.nottext)
...
router.get('/textmaker/banner65', async (req, res, next) => {
	...
	if(apikeyInput != 'ZailaniGans') return res.json(loghandler.invalidKey)
        if (!theme) return res.json(loghandler.nottheme)
        if (theme != 'yellow-roses' && theme != 'thunder') return res.json(loghandler.notheme)
        if (!text) return res.json(loghandler.nottext)
