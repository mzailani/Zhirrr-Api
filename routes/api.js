...
router.get('/textmaker/banner90', async (req, res, next) => {
        var theme = req.query.theme,
             text = req.query.text,
             text2 = req.query.text2,
             text3 = req.query.text3,
	...
	if(apikeyInput != 'ZailaniGans') return res.json(loghandler.invalidKey)
        if (!theme) return res.json(loghandler.nottheme)
        if (theme != 'firework' && theme != 'thunder') return res.json(loghandler.notheme)
        if (!text) return res.json(loghandler.nottext)
...
router.get('/textmaker/new2', async (req, res, next) => {
	...
	if(apikeyInput != 'ZailaniGans') return res.json(loghandler.invalidKey)
        if (!theme) return res.json(loghandler.nottheme)
        if (theme != 'smoke' && theme != 'cross-fire') return res.json(loghandler.notheme)
        if (!text) return res.json(loghandler.nottext)
